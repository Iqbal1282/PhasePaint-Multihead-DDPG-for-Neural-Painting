...
class MoEDDPG:
    ...
    def noise_action(self, action, target_theta=None, noise_factor=1.0):
        """
        Robust vectorized exploration for N strokes.
        action: [Batch, N * 13]
        target_theta: [Batch]
        """
        # 1. Vectorized Gaussian Jitter for all parameters
        # self.noise_level is [Batch], we broadcast to [Batch, N*13]
        ...
        # 2. Vectorized Directional Exploration (Option A)
        if target_theta is not None:
            B = action.shape[0]
            # Total parameters divided by 13 gives us N strokes
            N = action.shape[1] // 13
            
            target_theta_np = to_numpy(target_theta) 
            kappa = 5.0 / (noise_factor + 1e-6)
            
            # Sample direction from Von Mises centered on the Sobel edge [B]
            geo_samples = np.random.vonmises(mu=target_theta_np, kappa=kappa)
            
            # L is the displacement magnitude
            L = 0.15 
            
            # Reshape action to [Batch, N, 13] to easily access stroke-specific indices
            action_reshaped = action.reshape(B, N, 13)
            
            # Calculate new endpoints relative to start points (x0, y0)
            # x0 is index 0, y0 is index 1, x2 is index 4, y2 is index 5
            # geo_samples is [B], we use [:, np.newaxis] to broadcast across N strokes
            new_x2 = action_reshaped[:, :, 0] + L * np.cos(geo_samples[:, np.newaxis])
            new_y2 = action_reshaped[:, :, 1] + L * np.sin(geo_samples[:, np.newaxis])
            
            # Apply the updates
            action_reshaped[:, :, 4] = new_x2
            action_reshaped[:, :, 5] = new_y2
            
            # Flatten back to [Batch, N * 13]
    ...
    def update_policy(self, lr):
        ...
        _ , cur_q, step_reward = self._evaluate(state, action)

        # ── Critic update (TD) ────────────────────────────────────────────────
        with torch.no_grad():
            next_action = self.play(next_state, target=True)
            _, target_q, _ = self._evaluate(next_state, next_action, target=True)
            target_q    = self.discount * \
                          ((1 - terminal.float()).view(-1, 1)) * target_q + \
                          reward.view(-1, 1) + step_reward.view(-1, 1)
        ...
        pre_q, _, _    = self._evaluate(state.detach(), action_pred)
